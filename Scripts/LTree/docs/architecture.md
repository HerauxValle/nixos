...
| `util/` | `util.*` -- dependency-free shared helpers (`SBuf`, UTF-8 width, formatting); `spinner.*` -- the stderr-only, tty-gated loading spinner. |
...
| `scan.h` / `scan.c` | The one filesystem walk. `build_tree` recurses, applying `--exclude`/`--gitignore`/`-o HIDDEN`, respecting `-L`/`-d` (ls-mode forces effective depth 0 from `main.c`), and -- in a single `mmap` + linear pass per file -- counts lines, counts UTF-8 chars, hashes the file's bytes (if needed; `--simple-hash` samples size+first/last 64KiB instead of the whole file above 128KiB, via `hash_simple_or_full()`), and searches for a `-o DESC` marker (if needed, bounded to the first 64KiB, via `desc_search()`). Also fetches birth time (`fetch_btime`, `--sort birth`) via `statx()`. Directory hashes are combined from already-computed child hashes, no re-reading involved. Takes three optional streaming hooks (`on_dir_measure`/`on_entry_ready`/`on_dir_done`), interleaved into the recursion itself, that `main.c` only wires up when `--live` is passed. Also ticks the loading spinner (`util/spinner.h`) once per directory entry processed. |
...
| `spinner.h` / `spinner.c` | The animated "still working" indicator: `spinner_start`/`spinner_tick`/`spinner_erase`/`spinner_stop`, stderr-only and tty-gated (a no-op when stderr isn't a terminal, so non-interactive runs like `tools/smoke_test.sh` never draw anything), rate-limited to ~90ms between redraws unless forced. `scan.c` ticks it once per directory entry; `render_tree.c`'s `--live` path erases it before and redraws it after every streamed line so it stays the bottom-most line. `main.c` starts/stops it around the whole walk. |
| `colors.h` | The ANSI palette. `COL()`/`RST()` macros collapse to `""` under `--no-colour`, so print sites never branch on that flag themselves. |
| `json.h` / `json.c` | Two things in one file: (1) the JSON *writer* (`json_render`/`print_json`), used by both `-j` and `--save-output` so they never drift apart -- `json_key_allowed()` gates every optional top-level/per-entry field through `--stdout`'s exclusive/inclusive filter, and is also exposed (not `static`) so `main.c`'s `field_wanted()` can reuse the exact same rule to decide whether a `--stdout`-only-requested field (e.g. `HASH`/`DESC`) is worth actually computing; (2) a minimal recursive-descent JSON *reader*, just enough to parse ltree's own snapshot files back in for `-o DIFF` -- not a general-purpose validator. |
| `diff.h` / `diff.c` | `-o DIFF` support: finds the newest `.ltree/*.json` snapshot, flattens it into a sorted-by-path table (so comparison is a `bsearch`, not a parallel tree walk -- trees can differ in shape when files were added/removed), and marks `Node.modified` on anything that differs. `diff_peek_algo()` also reads back the snapshot's `hash_algo`/`hash_sampled` fields *before* the fresh scan runs, so `main.c` can force the current run's algorithm and `--simple-hash` setting to match -- comparing hashes computed two different ways would otherwise flag everything as modified. |
