...
static bool is_excluded(const Config *cfg, const GitTable *gt,
     ...
     * and diffed against themselves on the next run. */
        ...
        if (target_is_dir) {
            ...
            if (can_descend) {
                ...
                for (size_t i = 0; i < node->nchildren; i++) {
                    ...
                    node->size_bytes += node->children[i]->size_bytes;
                }
                if (need_hash && node->nchildren > 0) {
                    ...
                    for (size_t i = 0; i < node->nchildren; i++) {
                        names[i] = node->children[i]->name;
                        memcpy(hashes[i], node->children[i]->hash, HASH_MAX_BYTES);
                        lens[i] = node->children[i]->hash_len;
                    }
