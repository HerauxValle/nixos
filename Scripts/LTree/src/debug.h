...
/* Appends `"debug": { ... }` (with its own leading/trailing comma+
 * newline, matching the other top-level blocks in json_render) -- call
 * site must only call this when it actually wants the block present. */
