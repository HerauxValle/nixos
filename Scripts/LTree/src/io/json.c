/* &desc: "Implements json_render/print_json (the JSON writer, with json_key_allowed() gating every optional field by mirroring -o, same as the terminal views) and the minimal recursive-descent JSON reader used to load snapshots back in for -o DIFF." */
#define _GNU_SOURCE
#include "io/json.h"
#include <stdio.h>
#include <stdlib.h>
 ...
 * else funnels through this one predicate, which just mirrors -o
 * exactly, same "only what was actually asked for" contract the
 * terminal views already have (want everything? -oA. want everything
 * except one thing? -oA <exclude-list>, both already exist, no need
 * for a separate JSON-only mechanism). EXT and HIDDEN have no distinct
 * JSON field of their own (EXT only ever affects the tree view's
 * display name; JSON always uses full names) so enabling them is a
 * harmless no-op, not an error.
 *
 * TREE is the one exception: it's always included regardless of -o,
 * because for JSON (unlike the terminal) it isn't a "view" you opt
 * into -- the scan itself already stays fully recursive regardless of
 * -o TREE (see main.c), so the tree data always exists and JSON always
 * exports it. save.c's --save-output caller forces every
 * cfg->modules[] bit true on its own Config copy before calling this,
 * so a snapshot is always the full data regardless of what -o this run
 * asked for on its own stdout. ===================================== */
bool json_key_allowed(const Config *cfg, ModuleId id) {
    return id == MOD_TREE || cfg->modules[id];
}
...
void json_render(SBuf *sb, Node *root, const char *display_path,
    ...
    /* Always present (like hash_algo), never gated by json_key_allowed() -- a later
     ...
     * of its own) since -o filtering means any subset of them
 ...
 * so -o filters this exactly like the nested writer. ============= */
