...
/// btrfs's own real minimum formattable size is 114294784 bytes
/// (≈109.03 MiB usable) -- a floor of `LUKS_OVERHEAD_MB + 64` (96 MiB)
/// is below that, so a vault created or shrunk to exactly cas's
/// reported floor fails on first `mkfs.btrfs` with "too small to make
/// a usable filesystem" instead of the clean success the floor message
/// promises. `+ 96` clears btrfs's real minimum with a safety margin
/// (btrfs-progs' exact threshold can shift slightly by version) and
/// rounds to a clean 128 MiB usable.
pub const MIN_VAULT_MB: u64 = LUKS_OVERHEAD_MB + 96;
