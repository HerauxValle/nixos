...
pub struct Meta {
    ...
    /// Sha256 hex digest of the header wherever it currently lives,
    /// captured every time `check_lockout` confirms the passphrase
    /// actually opens the vault (or a header relocation/encryption/
    /// rotation commits — see `header::relocate::reseed_lockout_baseline`).
    /// Native-front: `luks::native_header_digest`, a digest of the fixed
    /// front region, meaningful independent of any secret. Relocated
    /// (`headerOffset`/`headerEncryption`): `header::relocate::
    /// room_slot_digest` of the raw room-slot bytes at `header_checksum_slot`
    /// (below) — the room-slot address is itself secret-derived, so unlike
    /// the native-front case it has to be pinned down at seed time rather
    /// than re-derived from whatever passphrase a later failed attempt
    /// happens to type in (a wrong passphrase derives an unrelated slot,
    /// which would false-flag as "corruption" on every ordinary
    /// wrong-passphrase attempt if it were re-derived from the attempt's
    /// own secret instead). Lets a later failed open tell "the header
    /// changed since then" (corruption) apart from "the header is
    /// unchanged, this passphrase is just wrong" — see
    /// `commands::open::check_lockout`. Not tamper-HMAC-covered: this is
    /// a corruption detector, not an attacker-facing protection toggle
    /// (same category as `sandbox_cgroup_mem`), and only ever compared
    /// against the header bytes themselves, never trusted on its own.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub header_checksum: Option<String>,
    /// The room-slot index `header_checksum` was captured from, when the
    /// header is relocated (`None`/unused for native-front). See
    /// `header_checksum`'s doc comment for why this has to be captured
    /// once at seed time instead of re-derived from whatever secret a
    /// later `open` attempt supplies.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub header_checksum_slot: Option<u64>,
