...
fn with_room_hidden<T>(img: &Path, f: impl FnOnce() -> Result<T>) -> Result<T> {
    with_boundary_hidden(img, room::container_boundary(img), f)
}

fn with_boundary_hidden<T>(img: &Path, boundary: u64, f: impl FnOnce() -> Result<T>) -> Result<T> {
    ...
    // A loop device's kernel-reported capacity doesn't update
    // synchronously with a `set_len` on its backing file -- there's a
    // separate, asynchronous udev/loop-driver capacity-change detection
    // (visible in `dmesg` as "loopN: detected capacity change"), which
    // can race `cryptsetup open`'s own two-phase (create table, then
    // reload it with final params) activation: confirmed live
    // 2026-08-17 -- the reload phase failed with "Not enough provided
    // sectors" because it saw a *different* device size than the create
    // phase did, moments apart. `refresh_size` (`losetup -c`) forces the
    // loop device to synchronously pick up the new size before `f()`
    // ever calls into cryptsetup.
    udisks::refresh_size(img);
...
// --- bruteforceLockout corruption-baseline support --------------------------

/// Which room slot `secret` currently maps to, or `None` if there's no
/// room/salt provisioned yet. Used only at seed time (a successful open,
/// or a relocation/encryption/rotation commit) when `secret` is already
/// known-correct -- `check_lockout`'s own failed-open path must never
/// call this with an unverified attempt's secret, since a wrong
/// passphrase maps to an unrelated slot (see `Meta::header_checksum`'s
/// doc comment).
pub fn current_room_slot(img: &Path, secret: &[u8]) -> Option<u64> {
    let salt = room::read_salt(img)?;
    let master = header::derive_master_secret(&[secret], &salt);
    Some(header::derive_slot_index(&master, room::N_SLOTS as usize) as u64)
}

/// sha256 hex of the raw (still-framed, still-encrypted-if-applicable)
/// bytes stored at room slot `index` -- the room-slot equivalent of
/// `luks::native_header_digest`, deliberately secret-independent (takes
/// an already-resolved slot index, not a secret to derive one from) so
/// `check_lockout` can re-check the *same* slot on every failed attempt
/// regardless of what passphrase was just typed in. `None` if there's no
/// room provisioned, or `index` is out of range.
pub fn room_slot_digest(img: &Path, index: u64) -> Option<String> {
    let buf = room::read_slot(img, index)?;
    let mut hasher = Sha256::new();
    hasher.update(&buf);
...
/// Re-seed the bruteforceLockout corruption-detection baseline
/// (`Meta::header_checksum`/`header_checksum_slot`, checked by
/// `commands::open::check_lockout`) right after a header
/// relocation/encryption/rotation commit, using the secret that
/// operation just verified. No-op if bruteforceLockout isn't enabled --
/// the baseline is meaningless otherwise, and there's no point paying
/// the extra digest/slot-derivation cost on every enable/disable/rotate
/// call for vaults that don't use this feature.
///
/// Why this has to run at every one of those transitions, not just at
/// `bruteforceLockout enable`: a baseline captured before headerOffset/
/// headerEncryption changes would go stale the instant the underlying
/// bytes move or get re-encrypted (a moved header changes location
/// entirely -- the *whole point* of `header_checksum_slot` -- and a
/// rotation moves which slot is in play even when the location doesn't).
/// A stale baseline is exactly as dangerous as the gap this whole fix
/// closes: it would make the *next legitimate* open look like corruption
/// (false positive) or, worse, silently stop protecting against real
/// corruption if the stale digest happens to still be present somewhere
/// unrelated. Called from every enable/disable path in this file and
/// from `relocate_if_enabled`'s rotation path, right before each of
/// their own `meta.write()` commits.
pub fn reseed_lockout_baseline(meta: &mut Meta, img: &Path, secret: &[u8]) {
    if meta.bruteforce_lockout != Some(true) {
        return;
    }
    if is_native_front(meta) {
        meta.header_checksum = luks::native_header_digest(img);
        meta.header_checksum_slot = None;
    } else if let Some(slot) = current_room_slot(img, secret) {
        meta.header_checksum = room_slot_digest(img, slot);
        meta.header_checksum_slot = Some(slot);
    } else {
        meta.header_checksum = None;
        meta.header_checksum_slot = None;
    }
}
...
fn enable_offset_inner(ctx: &Ctx, vault: &Vault, meta: &mut Meta, secret: &[u8], strength: Strength) -> Result<()> {
    ...
    meta.header_offset = Some(true);
    reseed_lockout_baseline(meta, &vault.img, secret);
...
pub fn relocate_if_enabled(ctx: &Ctx, vault: &Vault, meta: &mut Meta, old_secret: &[u8], new_secret: &[u8], strength: Option<Strength>) -> Result<()> {
    ...
    if offset_enabled(meta) {
        ...
        reseed_lockout_baseline(meta, &vault.img, new_secret);
