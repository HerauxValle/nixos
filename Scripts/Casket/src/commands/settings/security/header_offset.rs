// &desc: "`cas <vault> settings security headerOffset enable|disable|state` — relocates the vault's LUKS2 header from the container's front to a passphrase-derived slot inside the 32 MiB header-hiding room (see header/relocate.rs), so a magic-byte scan of the front no longer finds it. Direct-dispatch (not registry::Feature) since enable prints a conditional headerEncryption notice, following fileIntegrity.rs's pattern."
