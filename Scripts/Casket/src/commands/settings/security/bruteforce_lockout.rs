...
fn enable(ctx: &Ctx, vault: &Vault, n: Option<u32>, pw: Option<&str>) -> Result<()> {
    ...
    // Seed the corruption-detection baseline right away (see
    // `commands::open::check_lockout`) instead of waiting for the first
    // subsequent `open` to establish it -- a passphrase was just
    // verified here, so this is the earliest point a known-good header
    // digest can be captured. The relocated-header case needs the
    // verified secret to locate its room slot (see
    // `header::relocate::reseed_lockout_baseline`'s doc comment); the
    // native-front case doesn't, so it's still seeded even if `gate_inner`
    // decided this feature didn't require verification this time.
    if let Some((_, secret)) = &verified {
        header::relocate::reseed_lockout_baseline(&mut meta, &vault.img, secret);
    } else if header::relocate::is_native_front(&meta) {
