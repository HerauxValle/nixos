...
pub fn run(ctx: &Ctx, path_override: Option<&Path>) -> Result<()> {
    ...
    if let Ok(mounts) = std::fs::read_to_string("/proc/mounts") {
        for line in mounts.lines() {
            ...
            // Usable size, not raw on-disk -- matches `info`'s primary
            // `size` field (see `config::LUKS_DATA_OFFSET_MB`); the
            // 128 MiB reserve stays a labeled secondary line in `info`
            // rather than showing up here unexplained.
            let size_mb = (img.metadata()?.len() / (1024 * 1024)).saturating_sub(crate::config::LUKS_DATA_OFFSET_MB);
