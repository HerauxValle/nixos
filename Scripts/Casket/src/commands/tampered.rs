// &desc: "`cas <vault> tampered [--pass ...]` — on-demand tamper check: resolves and cryptographically verifies the real passphrase, then reports whether ransomwareProtection/verify_required/zeroize match the last passphrase-verified write. Always resolves a real passphrase (like auth passwd), since a check that could pass without one would also let an attacker forge a matching result without one."
...
pub fn run(ctx: &Ctx, vault: &Vault, pw: Option<&str>) -> Result<()> {
    ...
    match tamper::verify(&secret, &meta) {
        ...
        tamper::Status::Tampered => {
            logf!(ctx, "[x] '{}' tampered — ransomwareProtection/verify_required/zeroize don't match the last verified write", vault.name);
            logf!(ctx, "    run 'cas {} open' to reset those 3 settings to their safe defaults, or review them yourself first", vault.name);
        }
