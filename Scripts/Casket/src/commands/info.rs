...
pub fn run(ctx: &Ctx, vault: &Vault, pw: Option<&str>) -> Result<()> {
    ...
    // Raw on-disk size includes `config::LUKS_DATA_OFFSET_MB` (128 MiB),
    // a fixed reserve every vault carries whether or not headerOffset is
    // in use -- see `create.rs`'s `size_arg` and `Bugs/info-size-
    // includes-hidden-offset-unexplained.md`. Usable size (what `--size`
    // at create time actually asked for) is the number a user doing
    // capacity planning wants as the primary `size`; on-disk stays
    // visible as a labeled secondary line so the two never look like
    // unexplained drift from each other.
    let disk_mb = vault.img.metadata()?.len() / (1024 * 1024);
    let size_mb = disk_mb.saturating_sub(crate::config::LUKS_DATA_OFFSET_MB);
    ...
    // call together.
    let mut names: Vec<&str> = vec!["vault", "size", "size (on-disk)", "open", "slots", "passphrase", "keyfile", "keep", "threshold"];
    ...
    logf!(ctx, "{}", registry::kv_line("size (on-disk)", &format!("{disk_mb} MiB  ({} MiB reserve)", crate::config::LUKS_DATA_OFFSET_MB), width));
