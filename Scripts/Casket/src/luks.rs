...
/// Sha256 hex digest of the container's native-front header region --
/// the first `LUKS_DATA_OFFSET_MB` bytes, where the LUKS2 metadata and
/// keyslots live for every vault this build formats (`format_vault_ex`
/// always passes this exact `--offset`). Computed straight from the
/// file, independent of any passphrase — this is what lets a caller
/// distinguish "these header bytes changed since the last time this
/// secret was confirmed to open the vault" (corruption: bitrot, a bad
/// sector, a torn write) from "these bytes are unchanged, but this
/// passphrase doesn't match them" (a genuine wrong guess), which
/// cryptsetup's own "No key available with this passphrase" error
/// cannot tell apart on its own — see `commands::open::check_lockout`,
/// the only caller. `None` if the file is shorter than the header
/// region (should never happen for a real vault, but a truncated/mangled
/// file must never panic here).
pub fn native_header_digest(img: &Path) -> Option<String> {
    use std::io::Read;
    let region_len = (crate::config::LUKS_DATA_OFFSET_MB * 1024 * 1024) as usize;
