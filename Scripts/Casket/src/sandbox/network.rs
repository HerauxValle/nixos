...
pub fn setup_host_side(cfg: &Config) -> io::Result<Handle> {
    ...
    let result = (|| -> io::Result<()> {
        ...
        // See `enable_loopback_hairpin`'s own doc comment -- required for
        // the inbound DNAT `output`-hook rule (nat.rs's `CHAIN_IN_OUTPUT`)
        // to actually deliver traffic, not just get accepted by nftables.
        enable_loopback_hairpin(IFACE_HOST)?;
        Ok(())
    })();
    if let Err(e) = result {
        let _ = delete_link(rt_fd, IFACE_HOST);
...
pub fn setup_sandbox_side(handle: &Handle) -> io::Result<()> {
    ...
    let result = (|| -> io::Result<()> {
        ...
        // Same reasoning as the host side's own call -- see
        // `enable_loopback_hairpin`'s doc comment. Needed on *both* ends
        // of the veth: the host side lets the DNAT'd packet (still
        // carrying its original 127.0.0.1 source) actually get routed out
        // via `casnet0` at all; this side (`casnet0p`, inside the
        // sandbox's own netns) is what actually receives that packet and
        // has to accept/ARP-reply for a sender address in 127.0.0.0/8
        // instead of silently treating it as a bogus/martian source and
        // dropping it -- confirmed live via `tcpdump`/`arping`: without
        // this, the sandbox side never even ARP-replies to the host's
        // "who-has 10.200.99.2 tell 127.0.0.1", so the host gives up with
        // an ICMP host-unreachable before a single TCP packet is
        // exchanged.
        enable_loopback_hairpin(IFACE_PEER)?;
        Ok(())
    })();
    netlink::close(ns_fd);
    result
...
/// Enables `net.ipv4.conf.<iface>.route_localnet` AND
/// `net.ipv4.conf.<iface>.accept_local` -- both required, together, for
/// inbound forwarding (`nat.rs`'s DNAT rules) to work when the *client
/// itself is the host* connecting via one of its own loopback-range
/// addresses (`127.0.0.1`, the exact case in the bug report's repro, run
/// from a second shell on the same machine as `cas`). Confirmed to be
/// exactly these two sysctls and nothing else in nftables' own rule
/// construction by bisecting with `arping`/`tcpdump` directly against the
/// real veth pair, deliberately outside `cas` entirely: `arping -I
/// casnet0 -s 127.0.0.1 <SANDBOX_IP>` got zero replies with either
/// sysctl left at its kernel default (`0`) on either end of the link, and
/// started working the moment both were `1` on both ends -- this is a
/// kernel-level gate on 127.0.0.0/8 addresses entirely outside nftables'
/// own rule evaluation, not a rule construction bug, so it's invisible to
/// both `enable`'s own success message and even a byte-correct `nft list
/// ruleset` dump of what `cas` built.
///
/// Route_localnet alone (an earlier, incomplete fix attempt) was *not*
/// sufficient on its own, confirmed by isolating it from the also-present
/// `NF_INET_LOCAL_OUT` DNAT chain fix below: even with a real DNAT rule
/// on both `PREROUTING` and `OUTPUT`, and `route_localnet=1` on both
/// `casnet0` and `casnet0p`, a 127.0.0.1-sourced request to the sandbox's
/// veth address still got zero ARP replies. `accept_local` is the second,
/// separate piece -- `route_localnet` governs whether 127.0.0.0/8 is
/// treated as a routable (non-martian) *destination*; `accept_local`
/// separately governs whether a packet whose *source* address is itself
/// in a locally-owned range is accepted when it arrives on an ordinary
/// (non-loopback) interface rather than being dropped as spoofed. The
/// kernel's own ARP-reply path (`arp_process` in `net/ipv4/arp.c`) does a
/// full route lookup treating the ARP request's sender address as a
/// packet source before deciding whether to answer at all, so it hits
/// *both* checks.
///
/// Needed on **both** ends of the veth: the host side (`IFACE_HOST`,
/// called from `setup_host_side`) is what lets the DNAT'd packet (still
/// carrying its original 127.0.0.1 source) get routed out via `casnet0`
/// and get its ARP request answered at all; the sandbox side
/// (`IFACE_PEER`, called from `setup_sandbox_side`) is what lets that
/// packet actually be *accepted* once it arrives on `casnet0p` instead of
/// being silently treated as a spoofed/martian source.
///
/// Real, non-loopback source addresses (the bug report's other repro
/// case, `10.0.2.15`, and any genuinely remote client) never trigger
/// either check -- confirmed separately, that path worked once the
/// `output`-hook DNAT rule itself existed (see `nat.rs`'s
/// `CHAIN_IN_OUTPUT`) even before either of these sysctls was added.
/// Written directly on the specific interface, not `conf.all`/
/// `conf.default` -- each interface gets its own independent sysctl node
/// the moment it's created, so this needs no "restore old value"
/// bookkeeping the way `ip_forward` (a single host-wide, not
/// per-interface, knob) does: deleting the interface on teardown removes
/// the node with it.
fn enable_loopback_hairpin(iface: &str) -> io::Result<()> {
    std::fs::write(format!("/proc/sys/net/ipv4/conf/{iface}/route_localnet"), b"1")?;
    std::fs::write(format!("/proc/sys/net/ipv4/conf/{iface}/accept_local"), b"1")?;
    Ok(())
}
