...
/// 9. If `seccomp` is given, it is *not* applied here to the PID1 child
///    as a whole -- it's handed down to `reaper::run_as_pid1`, which
///    applies it only in the second fork it makes for the foreground
///    command, immediately before that child's own `execvp`. A custom
///    profile is meant to restrict what the *sandboxed command* can do,
///    not this supervisor's own bookkeeping (`getpid`/`wait4`/`kill`/
///    `fork`/`clone`/`exit_group`, but also, less obviously, `write`,
///    `mmap`, `rt_sigreturn`, `tgkill` and everything else the Rust
///    runtime and its own panic/error-reporting path need just to keep
///    functioning) -- applying it any earlier put the supervisor itself
///    under the same restrictive default-deny filter it was trying to
///    warn the user about, so a profile missing `write` (which most
///    custom profiles do, since nothing prompts a user to add it) left
///    the process unable to even print the error explaining what went
///    wrong: confirmed by strace against a `default: deny` profile with
///    an empty allow list -- `getpid()` correctly failed and the
///    reaper's own detailed error message got built, but the `write()`
///    call meant to print it also returned EPERM, which made Rust's
///    `println!` panic ("failed printing to stdout"), whose own panic-
///    reporting `write()` calls *also* hit EPERM, which escalated to
///    `tgkill(SIGABRT)` -- also blocked -- and finally an unreturnable
///    signal frame (`rt_sigreturn` itself blocked) that free-ran as a
///    SIGSEGV loop until an external timeout killed it: total silence,
///    not the intended error message, even though the check that was
///    supposed to produce it fired exactly as designed. Applying the
///    filter only around the foreground command's own `execvp` (see
///    `reaper::run_as_pid1`'s doc comment) means the supervisor never
///    has to satisfy someone else's allow list to report its own
///    problems, while the foreground command -- and everything *it*
///    forks/execs -- is still fully covered by the filter, inherited
///    across `execve` exactly as before.
...
pub fn run(
    ...
    if pid1_child == 0 {
        ...
        match reaper::run_as_pid1(argv, seccomp_filter) {
