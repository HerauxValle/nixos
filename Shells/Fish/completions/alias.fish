...
alias passwd-encryption="sudo cryptsetup luksChangeKey /dev/disk/by-uuid/80b7960d-fb8d-4dc3-8b01-329770c6e027" #&help:"Change root LUKS passphrase"
alias rebuild="pacnix rebuild 2>&1 | tee /tmp/pacnix-rebuild.log"       #&help:"Rebuilds nixos with pacnix"
alias pri="pacnix rebuild --impure 2>&1 | tee /tmp/pacnix-rebuild.log"  #&help:"Rebuilds nixos with pacnix (impure)"
...
alias moddoctor="~/Dotfiles/Scripts/Prism/modDoctor.sh"                 #&help:"Diagnose Prism Launcher mod crash (entrypoint/API mismatch)"
alias wadb="~/Dotfiles/Scripts/Waydroid/adb-connect.sh"                 #&help:"Start Waydroid, unfreeze if needed, and connect adb"
alias mcshadercache="for p in (pgrep -f 'minecraft-26.2-client'); echo \"=== PID \$p env ===\"; tr '\0' '\n' < /proc/\$p/environ 2>/dev/null | grep -E '__GL_'; end; echo ---; du -sh ~/.cache/nvidia/GLCache" #&help:"Check running Minecraft's NVIDIA shader cache env vars + cache dir size"
...
alias amcli="~/.gamdl/gamdl.sh"                                         #&help:"Apple Music downloader (gamdl+wrapper-v2) -- start/stop/auth/download"
