...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    if (checkHyprlandVersion(handle)) {
        if (!RenderHook::install(handle))
            HyprlandAPI::addNotification(handle, "[canvas] Disabled: couldn't find renderWindow/shouldRenderWindow to hook -- dispatchers still active", CHyprColor{1.0, 0.2, 0.2, 1.0},
                                          6000);
    }
