...
typedef CRegion (*renderPassRenderFn)(Render::CRenderPass*, const CRegion&);

static CRegion hkRenderPassRender(Render::CRenderPass* self, const CRegion& damage) {
...
static void hkRenderAllClientsForWorkspace(Render::IHyprRenderer* self, PHLMONITOR pMonitor, PHLWORKSPACE pWorkspace,
    ...
    if (g_pCanvas && g_pCanvas->isTransformed()) {
        ...
        CBox monitorBox = {0, 0, monSize.x, monSize.y};
        Render::GL::g_pHyprOpenGL->renderRect(monitorBox, CHyprColor(0.1, 0.1, 0.1, 1.0), {});
        ...
        g_pHyprRenderer->m_renderData.clipBox = {};  // no clip
