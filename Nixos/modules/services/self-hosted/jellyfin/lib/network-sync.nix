...
''
  ...
  KEY="$(api_key)"
  if [ -z "$KEY" ]; then
    echo "self-hosted-jellyfin-network-sync: no admin API key available yet (log into the dashboard, or set JELLYFIN_API_KEY via secrets self-hosted jellyfin) -- skipping"
    exit 0
  fi
