...
# SearXNG has two independent things that can go stale (the pinned core
# commit, the dependency lock), same shape as ComfyUI's update.nix minus
# the nodes part (no node/model ecosystem here).
#
# update            -- core, then deps.
# update:core       -- just the pinned SearXNG core commit (coreRev).
...
let
  ...
  mkCoreCheckScript = { apply ? false }: ''
    set -euo pipefail
    latest_rev="$(git ls-remote https://github.com/searxng/searxng HEAD | cut -f1)"
    if [ -z "$latest_rev" ]; then
      echo "self-hosted-searxng: could not check core (network issue?)" >&2
      exit 1
    fi
    if [ "$latest_rev" = "${cfg.coreRev}" ]; then
      echo "self-hosted-searxng: core is up to date (${cfg.coreRev})"
    else
      echo "self-hosted-searxng: core update available -- ${cfg.coreRev} -> $latest_rev"
  ''
  + (if apply then ''
      sed -i "s|^\([[:space:]]*\)coreRev = \"[^\"]*\";|\1coreRev = \"''${latest_rev}\";|" "${configFile}"
      echo "self-hosted-searxng: applied -- core ${cfg.coreRev} -> $latest_rev in ${configFile}. Rebuild + restart to actually check it out."
    fi
  '' else ''
