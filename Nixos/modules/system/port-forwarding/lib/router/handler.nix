...
# Per-connection handling -- buffer just the request headers, read
# Host:, look it up in ROUTES, then either 301-redirect to the real
# port (redirect mode) or byte-forward to it (default, "byte-forwarding
# mode" in pmg's own terms -- the browser never sees the real port).
# Mirrors pmg's own _router_handle/_router_pipe/_router_respond.
