...
# Snapshot + query tool for the machine's own public IPv4/IPv6
# addresses -- mirrors pmg's own ip_history_record/_ip_history_load/
# detect_usable_ips/detect_public_ipv6 and its `show changed`/
# `show ipv4|ipv6 --last[:n]` subcommands, in one file since it's
# compact enough (~110 lines) that splitting it further wouldn't
# clarify anything. Wired as a periodic systemd timer (record) plus a
# subcommand of the unified `port-forwarding` CLI (`port-forwarding
# history ...`) for the query side -- see port-forwarding.nix for both.
# History lives in a plain JSON file, same as pmg's own, chmod'd 644
# (not the DynamicUser default 600) so the manual query command can
# actually read it as a normal user.
