...
# Serves the CA cert + an iOS install guide over plain HTTP for 3
# minutes, dual-stack (v4+v6) so it works over the LAN bridge too --
# mirrors pmg's own _cert_serve almost verbatim (same page copy/style,
# "port-forwarding" swapped in for "pmg"). Temporarily opens the port via
# iptables directly (not a declared config.vars.ports entry) since this
# is a genuinely one-off, short-lived manual action, not a persistent
# exposure -- same spirit as pmg's own fw_open/fw_close bracketing,
# simplified to just iptables since that's confirmed this machine's
# actual firewall backend (nftables isn't enabled).

# syntax: python
''
  def _detect_usable_ips():
      # Same trick as lib/ip-history.nix's own detect_usable_ips --
