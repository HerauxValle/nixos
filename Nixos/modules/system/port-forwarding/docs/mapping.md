...
| `pmg list`/`status` | `systemctl status port-forwarding-*` / `journalctl -u port-forwarding-*` | The declared config already tells you what *should* exist; systemd tells you what actually does. No separate introspection command needed. |
