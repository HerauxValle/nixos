...
| IP history (`show changed`/`show ipv4\|ipv6 --last`) | `lib/ip-history.py` | No native equivalent. A systemd timer replaces pmg's on-demand-only recording (a machine that's never manually asked "did my IP change" never gets a snapshot in pmg's own version). |
