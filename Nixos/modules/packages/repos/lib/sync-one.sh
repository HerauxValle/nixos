...
initial_branch="$(jq -r '.initialBranch // empty' <<< "$entry_json")"
user_name="$(jq -r '.userName // empty' <<< "$entry_json")"
user_email="$(jq -r '.userEmail // empty' <<< "$entry_json")"
signing_key="$(jq -r '.signingKey // empty' <<< "$entry_json")"
gpg_sign="$(jq -r 'if .gpgSign == null then empty else (.gpgSign | tostring) end' <<< "$entry_json")"
hooks_path="$(jq -r '.hooksPath // empty' <<< "$entry_json")"
excludes_file="$(jq -r '.excludesFile // empty' <<< "$entry_json")"
...
_set_remote() {
  ...
  if git -C "$path" remote get-url "$remote_name" >/dev/null 2>&1; then
    git -C "$path" remote set-url "$remote_name" "$remote_url" || fail=1
  else
    git -C "$path" remote add "$remote_name" "$remote_url" || fail=1
  fi
}
...
done < <(jq -r '.remotes | to_entries[] | "\(.key)\t\(.value)"' <<< "$entry_json")
...
if ! git -C "$path" rev-parse --verify -q HEAD >/dev/null; then
  if git -C "$path" fetch --quiet origin; then
    ...
    if [[ -z "$branch" ]]; then
      ...
      fail=1
    elif git -C "$path" checkout -B "$branch" "origin/$branch" --; then
...
[[ -n "$user_name" ]] && { git -C "$path" config --local user.name "$user_name" || fail=1; }
[[ -n "$user_email" ]] && { git -C "$path" config --local user.email "$user_email" || fail=1; }
[[ -n "$signing_key" ]] && { git -C "$path" config --local user.signingKey "$signing_key" || fail=1; }
[[ -n "$gpg_sign" ]] && { git -C "$path" config --local commit.gpgSign "$gpg_sign" || fail=1; }
[[ -n "$hooks_path" ]] && { git -C "$path" config --local core.hooksPath "$hooks_path" || fail=1; }
[[ -n "$excludes_file" ]] && { git -C "$path" config --local core.excludesFile "$excludes_file" || fail=1; }

while IFS=$'\t' read -r cfg_key cfg_value; do
  [[ -z "$cfg_key" ]] && continue
  git -C "$path" config --local "$cfg_key" "$cfg_value" || fail=1
done < <(jq -r '.extraConfig | to_entries[] | "\(.key)\t\(.value)"' <<< "$entry_json")
