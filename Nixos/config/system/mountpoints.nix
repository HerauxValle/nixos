...
# Get a disk's UUID with `lsblk -o NAME,MOUNTPOINTS,UUID,LABEL`.
{
  config.vars.system.mountpoints = {
    ...
    device = {
      ...
      # config.vars.system.mountpoints.device.<key>.path instead of hardcoding
      # /run/media/${config.vars.identity.username}/<label>. storage/media are
      # blocking = true since qbittorrent/jellyfin/stash all actually
      # read config.vars.system.mountpoints.device.storage.path -- a silent
      ...
      backup = {
        ...
        at = "/home/${config.vars.identity.username}/Drives";
        as = "Backup";
        owner = "${config.vars.identity.username}:users";
        blocking = false;
      };
      ...
      #   owner = "${config.vars.identity.username}:users"; # optional -- omit to leave as-is (root)
