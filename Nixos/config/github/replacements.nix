...
# modules/backup/dotfiles/default.nix for what each option actually does.
{
  config.vars.backup.dotfilesBackup = {
    ...
    replaceValues = [
      ...
      { file = "Nixos/config/config.nix"; key = "vars.identity.gitCommitEmail"; replaceWith = "maxmustermann@example.com"; }
      { file = "Nixos/config/config.nix"; key = "vars.security.usbKillswitch.usbSerialShort"; replaceWith = "0000000000000000000"; }
